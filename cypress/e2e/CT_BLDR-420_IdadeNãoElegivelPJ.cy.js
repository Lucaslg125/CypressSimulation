...
it('CT BLDR-420 PJ Idade não Elegivel para Seguro', () => {
  ...
  cy.get(el.DADOS_EMPRESA.TEL_EMP).type('8133201126')
  cy.get(el.DADOS_EMPRESA.EMAIL_EMP).type(email)

  cy.get(el.DADOS_EMPRESA.CEP_EMP).type('71200‑010{enter}').should('have.value', '71200‑010')

  cy.wait(2000)

  cy.get(el.AVALISTA_CNPJ.CPF).type(randomcpf)
  cy.get(el.CLIENTE.CAMPO_NOME).type(name)
  cy.get(el.AVALISTA_CNPJ.DTNASCIMENTO, { timeout: 80000 }).type('10/01/1940{Esc}')
  cy.get(el.CLIENTE.CAMPO_TEL).type('61997250105')
  ...
  cy.get(el.SIMULACAO.ENTRADA_CARD1, { timeout: 80000 }).should('be.visible').clear().type('75,00')
  ...
  cy.get(el.DADOS_CNPJ.FATURAMENTO_MENSAL, { timeout: 70000 }).type('180000000')
  ...
  cy.get(el.FICHA_DADOS_VEICULO.CHASSI, { timeout: 80000 }).type('70R 5YHkxv 4A 0K8106')
  cy.wrap('70R 5YHkxv 4A 0K8106').should('contain', '70R 5YHkxv 4A 0K8106')
  cy.wait(5000)
  ...
  cy.get(el.AVALISTA_1.DATA_DE_EMISSAO).type('19/07/2002{Esc}')
  ...
  cy.get(el.AVALISTA_1.NOME_MAE).type('Beatriz Mendes Silva Neves')
  ...
  cy.get(el.AVALISTA_1.NUMERO_ENDERECO, { timeout: 80000 }).type('505', { force: true }).should('have.value', '505')
  cy.wait(5000)

  //Dados Profissionais

  cy.contains('h2', 'Dados Profissionais do Avalista 1', { timeout: 70000 }).click({ force: true })
  cy.get(el.AVALISTA_1.NOME_DA_EMPRESA, { timeout: 70000 }).type('BF1 Advogados Associados', { force: true })//nome da empresa
